...
import br.com.cams7.app.event.AbstractEvent;
import br.com.cams7.app.event.AbstractEventListener;
...
public abstract class AbstractController implements ActionListener, WindowListener {
	...
	private Map<Class<?>, List<AbstractEventListener<?>>> eventListeners = new HashMap<Class<?>, List<AbstractEventListener<?>>>();
	...
	protected void fireEvent(AbstractEvent<?> event) {
		if (eventListeners.get(event.getClass()) != null) {
			for (AbstractEventListener eventListener : eventListeners.get(event.getClass())) {
				log.debug("Evento: " + event.getClass().getName() + " com listener: "
